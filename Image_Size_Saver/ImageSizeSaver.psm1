function GetMainMenuItems
{
    ...
    $menuItem1.MenuSection = "@Image Size Saver"
...
function Invoke-ImageCacheSizeSaver
{
    ...
    if ($PlayniteApi.Paths.IsPortable -eq $true)
    {
        ...
        $PathCacheDirectory = Join-Path -Path $PlayniteApi.Paths.ApplicationPath -ChildPath "library\files\*\*"
    }
    else
    {
        $__logger.Info("Image Cache Size Saver - Playnite is Installed.")
        $PathCacheDirectory = Join-Path -Path $env:APPDATA -ChildPath "library\files\*\*"
    }
    ...
    $ImageTempPath = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath 'ImageCacheSizeSaver.tmp'
